...
$id_usuario = 1; // Cambia esto por el ID del usuario correspondiente

if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    if (isset($_POST['delete_ids']) && is_array($_POST['delete_ids'])) {
        $ids_to_insert = implode(",", array_map('intval', $_POST['delete_ids']));
        $fecha_prestado = date('Y-m-d H:i:s');

        // Insertar en la tabla de alquileres
        foreach ($_POST['delete_ids'] as $id_coche) {
            $sql = "INSERT INTO alquileres (id_usuario, id_coche, prestado, devuelto) VALUES ($id_usuario, $id_coche, '$fecha_prestado', NULL)";
            if (!mysqli_query($conn, $sql)) {
                echo "<h1>Error al insertar el coche con ID $id_coche en alquileres: " . mysqli_error($conn) . "</h1>";
            }
        }
        echo "<h1>Coches alquilados correctamente</h1>";
        echo "<div class='volver'><a href='borrar.php' class='clasess'>Volver</a></div>";
    } else {
        echo "<h1>No se seleccionaron coches para alquilar</h1>";
        echo "<div class='volver'><a href='borrar.php' class='clasess'>Volver</a></div>";
    }
} else {
    ...
    if (mysqli_num_rows($result) > 0) {
        ...
        echo "<div style='text-align: center; margin-top: 20px;'>
            <button type='submit'>Alquilar seleccionados</button>
        </div>";
        echo "</form>";
    } else {
